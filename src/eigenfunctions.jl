...
using LaTeXStrings
    ...
    #Initial condition
    #pars = (mu=x->1,EI=x->1,q=x->-1) # changes
    #BCs  = Dict((0,'H')=>0,
    #            (0,'G')=>0,
    #            (1,'M')=>0,
    #            (1,'Q')=>0)
    pars = (mu=x->1,EI=x->1,q=x->0)
    BCs  = Dict((0,'H')=>1,
                (0,'G')=>0,
                (1,'M')=>0,
                (1,'Q')=>0)
    ...
    IC   = [sys.Se\sys.qe zeros(sys.shape[2])] # added
...
#    Plots.savefig(plt, "img/single/eigenfunctions_analytical_cantilever.svg")
    ...
    n = 4
    times = collect(LinRange(0, 3, 1000)) #collect(LinRange(0,0.1,100)) # changed
    eigenfunc_t = [
    ...
    #        label = L"w(x)", linewidth=2, color="blue")
    #    Plots.frame(numerical_anim, plt)
    #end
    get_sol = Beam1D.solve_dy_eigen(sys, 4)
    sol = get_sol(IC)

    numerical_anim = Plots.@animate for t in times
        Plots.plot([0,1],[0,0],color="black",label=false,linewidth=2,linestyle=:dot)
        Plots.plot!(sol(t)...,0,1,ylim=[-0.1,0.1],color="black",label=false,linewidth=2)
    end
